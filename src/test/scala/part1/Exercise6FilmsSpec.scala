...
class Exercise6FilmsSpec extends AnyFlatSpec with Matchers {
  ...
  "directorsBornBefore" should "work as expected" in {
    ...
    directorsBornBefore(directors, 1931) should contain theSameElementsAs List(
      eastwood
    )
    directorsBornBefore(directors, 1951) should contain theSameElementsAs List(
      eastwood
    )
    directorsBornBefore(directors, 1952) should contain theSameElementsAs List(
      eastwood,
      mcTiernan
    )
  ...
  "directorsBornBeforeWithBackCatalogOfSize" should "work as expected" in {
    // pending
    directorsBornBeforeWithBackCatalogOfSize(directors, 1931, 5) should contain theSameElementsAs List(
      eastwood
    )
  ...
  "namesOfFilms" should "work as expected" in {
    // pending
    namesOfFilms(List(memento, darkKnight)) should contain theSameElementsAs List(
      memento.name,
      darkKnight.name
    )
  ...
  "namesOfFilmsByDirector" should "work as expected" in {
    // pending
    namesOfFilmsByDirector(nolan) should contain theSameElementsAs nolan.films
      .map(_.name)
    namesOfFilmsByDirector(eastwood) should contain theSameElementsAs eastwood.films
      .map(_.name)
  ...
  "namesOfFilmsByDirectorScoringAtLeast" should "work as expected" in {
    // pending
    namesOfFilmsByDirectorScoringAtLeast(nolan, 8.8) should contain theSameElementsAs List(
      darkKnight.name,
      inception.name
    )
    namesOfFilmsByDirectorScoringAtLeast(nolan, 8.9) should contain theSameElementsAs List(
      darkKnight.name
    )
